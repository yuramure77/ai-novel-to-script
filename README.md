...
🌐 <strong>在线体验：http://118.25.85.229:8080 / https://www.2scrpt.art</strong>（备案审核中）
...
### 📂 项目管理
- **文件夹分组** — 自定义文件夹颜色标识，拖拽移动项目
- **角色筛选** — 全部 / 👑创建者 / 🔧管理员 / 👁只读，四档金色胶囊切换
- **项目搜索** — 标题实时模糊搜索，金色圆角搜索框
- **批量操作** — 多选后一键批量删除或移动到指定文件夹
- **卡片信息栏** — 标题两行截断 + 信息自动沉底，同一排等高对齐
- **重复提醒** — 新建项目名称重复时优雅提示，不再弹数据库错误

### 👥 多人协作
- **邀请链接** — 生成分享链接，协作者一键加入
- **权限分级** — 管理员（编辑/生成/管理协作者）vs 只读（仅查看剧本和原文）
- **活跃检测** — 协作者在线状态实时显示，权限标签「🔧 协管」「👁 只读」
- **安全保护** — 只读用户 API 层双重校验，无法绕过前端限制

### 🎨 AI 图像生成
- **DeepSeek 造型师** — 个性化外貌描写（年龄/性别/人种 8 类识别 + 性格→9 种视觉元素转化），再送入生图模型
- **人种感知** — 东亚/日本/朝鲜/欧洲/南亚/中东/东南亚/非洲/拉丁，自动匹配面部结构、肤色、瞳色、发型
- **混元 hy-image-lite** — 腾讯混元大模型图生，TokenHub API；生图限流 10 次/分钟防滥用
- **COS 永久存储** — 生成图片自动上传腾讯云对象存储，返回永久 URL
- **图片版本历史** — 每次生图独立存档，画廊支持回滚切换
- **场景图渐进式生成** — 后台 4 线程池异步执行，SSE 实时推送
- **熔断保护** — 连续失败 5 次自动暂停 60 秒；并发限制 JobNumExceed 自动重试
...
- **毛玻璃材质** — 弹窗、侧栏标题 backdrop-filter 毛玻璃 + 金色描边
- **可拖拽面板** — 原文/剧本/AI助手三栏手柄自由调整宽度
- **全局快捷键** — Ctrl+F 搜索、Ctrl+Enter 生成、Ctrl+S 保存、Esc 关闭
- **金色视觉系统** — 按钮/标签/tabs/输入框全部金色主题，登录页标签栏同款
- **响应式适配** — 手机端完整适配，弹窗宽度自适、表单移动端优化
...
| 提交分布 | 修复 74 + 新增 30 + 优化 28 + 文档 17 + 测试 6 + 重构 5 + 其他 36，合计 196 |
