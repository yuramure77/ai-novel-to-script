...
Schema 的 `scenes[].chapter` 字段承接上游分章引擎的输出。引擎采用 7 级回退策略：

| 优先级 | 匹配模式 | 示例 |
|--------|---------|------|
| 1 | 中文序数章 | 第一章、第二章 |
| 2 | 英文 Chapter | Chapter 1, Chapter 2 |
| 3 | 部/篇/卷 | 第一部、上卷 |
| 4 | 罗马数字 | I, II, III, IV |
| 5 | 日语数字 | 一、二、三 |
| 6 | 纯数字分隔 | 1. 2. 3. |
...
每识别到一个章节标题，引擎创建一条 `scenes` 记录，`chapter` 字段为 1-based 序号。后续 AI 生成时按 `chapter` 分组，新场景直接追加到列表末尾——这是 Schema 选择扁平列表而非嵌套结构的关键原因：增量生成无需修改父节点。
